...
You will first need OpenMPI_ in your computer. There are other MPI
...
``mpicpp`` in the ``globals.mk`` file. Once you are ready, simply run::

   cd nonpareil # or wherever you have the nonpareil folder
   make nonpareil-mpi

That's it. Now you should have the ``nonpareil-mpi`` binary, that you can place
in a location listed in your ``$PATH`` if you want.

Running Nonpareil MPI
---------------------

1. Get your machines ready. If you are familiar with MPI skip directly to #3. If
...
2. Obtain the machine names. Just prepare a raw text file with the list of
...
3. Run Nonpareil MPI. All you need is to call ``nonpareil-mpi`` with ``mpirun``.
...
If you are interested on MPI, I'm assuming you have big files, so you may be
also concerned about resources allocation.
