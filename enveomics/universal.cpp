...
int      Verbosity = 1;
ofstream LogH;
bool     OpenLog, CreateTmpDir = true;
...
std::filesystem::path tmp_dir() {
  if (!CreateTmpDir) return(TmpDir);

  char *subdir;
  subdir = new char[LARGEST_STRING];
  snprintf(subdir, LARGEST_STRING, "enveomics.%d", getpid());

  TmpDir = std::filesystem::temp_directory_path() / subdir;
  std::error_code ec;
  std::filesystem::create_directory(TmpDir, ec);
  if (ec) error("Cannot create temporary directory", TmpDir.c_str());
  return(TmpDir);
}
