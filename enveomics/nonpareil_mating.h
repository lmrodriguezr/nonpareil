 ...
 * Elements:
 *   - `double overlap`: Minimum overlap of the two reads, as a fraction of the
 *     shortest
 *   - `double similarity`: Minimum similarity, as a fraction of the overlapping
 *     region
 ...
 *   - `size_t number`: Number of elements in the block to mate (size of the
 *     region)
 *   - `int from_in_result`: Position of the initial element of the block in the
 *     query set (start of the block)
 *   - `matepar_t par`: Parameters of the matting
 *   - `int **results`: Reference to an int array to be filled with the matting
 *     results
 *   - `pthread_mutex_t *mutex`: Reference to a mutex to be used when modifying
 *     the array pointed by result
 *   - `char ***blockA`: Reference to an array of char arrays containing the
 *     reads in Block A
 *   - `char ***blockB`: Reference to an array of char arrays containing the
 *     reads in Block B
 ...
 *       unsigned int largest_seq, [unsigned int q_largest_seq,]
 *       matepar_t matepar);
 * Description:
 *   Counts all the mates in the dataset by blocks determined by the ammount of
 *   RAM allowed to use.
 * Input:
 *   - `int *&result`: Array of integers to be filled with the results of mating
 *   - `char *file`: File containing the sequences (assumed to be in
 *     "enveomics-seq" format)
 *   - `char *q_file` (optional): File containing the query sequences (assumed
 *     to be in "enveomics-seq" format). If not passed, `file` is used as query
 *     file
 *   - `int threads`: Number of threads (CPUs) to use
 *   - `unsigned int lines_in_ram`: Maximum number of sequences to be stored in
 *     RAM
 *   - `unsigned int total_seqs`: Total number of sequences in the file
 *   - `unsigned int largest_seq`: Length of the largest sequence
 *   - `unsigned int q_largest_seq` (optional): Length of the largest query
 *     sequence. Required if `q_file` is passed
 *   - matepar_t matepar: Parameters for the mating
 * Output:
 *   `size_t`: The number of slots in the results array (equal to the number of
 *   sampled queries)
 */
size_t nonpareil_mate(
 ...
 * Input:
 *   - `int *&result`: An int array to be filled with the number of mates per
 *     query read
 *   - `int from_in_result`: Position of the initial read in blockA, with
 *     respect to all the reads in the query set.  Also, the initial position in
 *     the result array to be filled
 *   - `char **&blockA`: Array of chr arrays containing the block of query
 *     sequences to mate
 *   - `char **&blockB`: Array of chr arrays containing the block of subject
 *     sequences to mate
 *   - `int sizeBlockA`: Number of elements in blockA
 *   - `int sizeBlockB`: Number of elements in blockB
 *   - `int threads`: Number of threads to use
 *   - `matepar_t matepar`: Parameters of the matting passed in a `matepar_t`
 *     complete object
 */
void nonpareil_count_mates_block(
      int *&result, int from_in_result, char **&blockA, char **&blockB,
 ...
 *   - `int talk`: The period (in number of query sequences) to report advance
 *      via `say()`.  Zero (`0`) to be silent
 *   - `matepar_t matepar`: A complete matepar_t object containing the matting
 *     parameters
 */
void nonpareil_count_mates(
      int *&result, char **&blockA, char **&blockB, int fromA, int numberA,
 ...
 * Description:
 *   Compares two reads, the first of which is shorter or equal in length to the
 *   second
 * Input:
 *   - `char *seqA`: Read A
 *   - `char *seqB`: Read B
 *   - `int lenA`: Length of seqA
 *   - `int lenB`: Length of seqB.  Notice that you *MUST* ensure that
 *     `lenA <= lenB`, the function doesn't check!
 *   - `matepar_t matepar`: Parameters of the comparison
 * Output:
 *   `bool`: True if the two sequences are similar, false otherwise
 */
