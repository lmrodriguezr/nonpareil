 ...
 *   - `pthread_mutex_t *mutex`: A mutex to be used when modifying the result
 *     array
 ...
 * Input:
 *   - `double *&result`: A pointer to a double array, to be filled with the
 *     results of the sub-samples
 *   - `int threads`: Number of threads to use in the sub-sampling process
 *   - `samplepar_t samplepar`: Parameters of the sub-sampling.  It's expected
 *     to be fully defined
 * Output:
 *   `int`: Number of samples taken.  This should be equal to the replicates
 *   element of samplepar
 */
int nonpareil_sample_portion(
      double *&result, int threads, samplepar_t samplepar);
 ...
 * Description:
 *   Performs the sub-sampling in a region of the matting resuts array.  All the
 *   parameters are passed though a reference to a samplejob_t element to allow
 *   threaded execution.
 * Input:
 *   - `void *samplejob_ref`: A reference to a complete `samplejob_t` object.
 * Output:
 *   `void *`: A reference to a zero.  Otherwise, a reference to an error
 *   message.
 */
void *nonpareil_sample_portion_thr(void *samplejob_ref);

/**
 * sample_t nonpareil_sample_summary(
 *       double *&sample_result, int sample_number, char *alldata,
 *       char *outfile, samplepar_t samplepar);
 * Description:
 *   Calculates and saves the summary of a sub-sampling.
 * Input:
 *   - `double *&sample_result`: A double array containing sub-sampling results,
 *     as produced by `nonpareil_sample_portion()`
 *   - `int sample_number`: Number of elements in `sample_result`
 *   - `char *alldata`: A char array with the path to the file in which all-data
 *     report must be created.  It can be an empty string (`""`), to ignore the
 *     all-data report
 *   - `char *outfile`: Char array with the path to the file in which the
 *     summary report must be created.  It can be a dash (`"-"`) to send the
 *     summary to the stdout, or an empty string (`""`) to ignore the report
 *   - `samplepar_t samplepar`: A `samplepar_t` object with the sub-sampling
 *     parameters.  It's expected to be complete
 * Output:
