...
int nonpareil_sample_portion(
  ...
  say("8sfs$", "Sampling at ", samplepar.portion * 100, "%");
  ...
  say("9sis$", "Gathered ", threads, " threads");
...
void *nonpareil_sample_portion_thr(void *samplejob_ref) {
  ...
  for (int i = 0; i < samplejob->number; i++) {
    ...
    for (int j = 0; j < samplejob->samplepar.mates_size; j++) {
      ...
      if (
            ((double)rand_r(&samplejob->rseed) / (double)RAND_MAX) <
            samplejob->samplepar.portion
          ) {
        sample_size++;
        // Does the sample contain at least one mate?
        n = (int)floor(
          (samplejob->samplepar.total_reads) * samplejob->samplepar.portion
        ) - 1;
        // -1 because we exclude the query read from the sample

        if (n < 0) n = 0;
        p = (double)(mates_ref[j] - 1.0) / (
          samplejob->samplepar.total_reads - 1.0
        );
        // Again, -1 in both terms because of the query read

        p_gt_0 = 1.0 - pow(1.0 - p, n);
        if ((double)rand_r(&samplejob->rseed) / (double)RAND_MAX < p_gt_0)
          found++;
      }
    }
  ...
  for (int i = 0; i < samplejob->number; i++)
    result_ref[i + samplejob->from_in_result] = result_cp[i];
  //           ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^--> position + first of the thread
...
sample_t nonpareil_sample_summary(
  ...
  if (alldata && strlen(alldata) > 0) {
    ...
    if (alldatah == NULL)
      error("I cannot write on all-data file", alldata);
    reportAllData = true;
  }
