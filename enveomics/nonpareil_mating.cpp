...
size_t nonpareil_mate(int *&result,
		...
		matepar_t matepar){
   // Vars
   int		no_blocks_sbj, no_blocks_qry,
   		no_seqs_block_qry, no_seqs_block_sbj,
		tmp_ram, result_i=0,
   ...
   // Mating
   say("3sisis$", "Mating sequences in ", no_blocks_qry, " by ", no_blocks_sbj, " blocks");
   for(int i=0; i<no_blocks_qry; i++){
      ...
      say("5sisi$", "Allocating ~", tmp_ram, " Mib in RAM for block qry:", i+1);
	 ...
	 say("5sisi$", "Allocating ~", tmp_ram, " Mib in RAM for block sbj:", j+1);
	 ...
	 // Mate
	 say("4sisi$", "Computing block ", (i+1)*(j+1), "/", no_blocks_qry*no_blocks_sbj);
...
void nonpareil_count_mates_block(int *&result, int from_in_result,
   ...
   // Launch jobs
   say("5sis$", "Launching parallel comparisons to ", threads, " threads");
...
void *nonpareil_count_mates_thr(void *matejob_ref){
   ...
   pthread_mutex_lock( matejob->mutex );
      say("4sisis>", "Thread ", matejob->id, " completed ", matejob->number," comparisons, joining results");
...
void nonpareil_count_mates(int *&result, char **&blockA, char **&blockB,
   ...
   for(int i=0; i<numberA; i++){
      if(talk>0 && i%talk==0) say("4sfs^", "Searching sequences: ", (double)i*100/(double)numberA, "% of the block");
